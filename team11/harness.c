...
/**
 * List of our MAMakefileC addresses
 */
                  ...
                  uint16_t cls3)
{
 if (ifc != 0) 
 {
   fprintf(stderr, "Received normal frame instead of control message\n");
   return 2;
 }

 if (cls3 == ifc) fprintf(stderr, "IFC OK\n");
 ...
 if ( (cls3 == ifc) &&
  ...
  for (unsigned int i = 0; i < msg_len; i++)
  {
    fprintf(stderr, "%d ", b1[i]);
  }
  fprintf(stderr, "\n");
  fprintf(stderr, "Is :\n");
  for (unsigned int i = 0; i < msg_len; i++)
  {
    fprintf(stderr, "%d ", b2[i]);
  }
  fprintf(stderr, "\n");
...
/**
 * Set source MAC in @a frame to the MAC of
 * interface @a ifc_num.
 *
 * @param frame[in,out] Ethernet frame, must be large enough
 * @param ifc_num identifies which MAC we should use
